...
export const user = () => {
  ...
  setInterval(async() => {
    ...
    infra4({ttl: 2, diff: true}).user.getByLocate({xxx, yyy}).then(users => {
      const filtered = users.filter(user => isExpiration(user.update, 30) && !peerManager?.peerMap[user.id])
      const usersObj = aToO(filtered, ({id, ...v}) => [id, v])
      if(!isEmptyO(usersObj)) addUser(usersObj)// RTCは除く
    })

    const [x, y] = getCamera()
    id && infra4({throttle: 1}).user.setLocate({id, x: x.toFixed(3), y: y.toFixed(3)})
    //定期的に古いの削除する
  }, 5000)
