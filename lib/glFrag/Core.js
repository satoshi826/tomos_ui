import {oForEach, keys, range} from '../util/index'
...
export class Core {
  ...
  createInstancedVbo(id, att, array) {
    this.useVao(id)
    const isUnitAtt = typeof strideMap[att] === 'number'
    let vbo = this.gl.createBuffer()
    this.gl.bindBuffer(this.gl.ARRAY_BUFFER, vbo)
  ...
  updateInstancedVbo(id, att, array, vbo) {
    this.useVao(id)
    this.gl.bindBuffer(this.gl.ARRAY_BUFFER, vbo)
...
export const strideMap = {
  a_position              : 3,
  a_textureCoord          : 2,
  a_instance_postPosition : 2,
  a_instance_postLuminance: 1
}
